...
public class ApplesAndPears {
	...
	public int getArea(String[] board, int K) {
		...
		for (char ch : chars) {
			...
			for (int i = 0; i < size; i++) {
				...
				for (int j = 0; j < size; j++) {
					...
					toFill[i + 1][j + 1] = value + toFill[i][j + 1] + toFill[i + 1][j] - toFill[i][j];
				}
			}
		}
		...
		int totalP = pearMatrix[size][size];
			...
			for (int y0 = 0; y0 < size; y0++) {
				for (int x1 = x0 + 1; x1 <= size; x1++) {
					for (int y1 = y0 + 1; y1 <= size; y1++) {
						...
						int apples = getCount(appleMatrix, x0, y0, x1, y1);

						if ((totalA >= area) && (pears == 0 || totalDot > 0)) {   //apple area
							if (dots + 2 * pears <= K) {
								if (max < area) {
									max = area;
								}
						...
						if ((totalP >= area) && (apples == 0 || totalDot > 0)) { //pear area
							if (dots + 2 * apples <= K) {
								if (max < area) {
									max = area;
								}
						...
						if ((totalDot >= area) && (apples + pears <= K)) { // dots area
							if (max < area) {
								max = area;
							}
						}
